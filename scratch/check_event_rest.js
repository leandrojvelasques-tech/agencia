...
async function checkEvent() {
  const response = await fetch(`${supabaseUrl}/rest/v1/events?id=eq.${eventId}&select=title,slug,event_materials(title,url)`, {
    headers: {
      'apikey': supabaseKey,
  ...
  console.log('Event Info:', JSON.stringify(data, null, 2))
}
