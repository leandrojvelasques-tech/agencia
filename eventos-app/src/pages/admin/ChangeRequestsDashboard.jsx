...
const emptyForm = { client_id: '', title: '', description: '', affected_area: '', request_type: 'improvement', category: 'other', priority: 'normal', status: 'initiated', billing_status: 'pending', internal_note: '', client_note: '' }
...
export default function ChangeRequestsDashboard() {
  ...
  const [filters, setFilters] = useState({ status: 'all', client: '', billing: 'all', category: 'all', query: '' })
  ...
  const filtered = useMemo(() => requests.filter(item => {
    const text = `${item.request_code || ''} ${item.title} ${item.description} ${item.requester_name || ''} ${item.requester_email || ''} ${item.crm_clients?.name || ''}`.toLowerCase()
    return (filters.status === 'all' || item.status === filters.status) && (filters.client === '' || item.client_id === filters.client) && (filters.billing === 'all' || item.billing_status === filters.billing) && (filters.category === 'all' || (item.category || 'other') === filters.category) && (!filters.query || text.includes(filters.query.toLowerCase()))
  }), [requests, filters])
  ...
  const save = async (event) => {
    ...
    delete payload.id; delete payload.created_at; delete payload.updated_at; delete payload.crm_clients; delete payload.attachments; delete payload.request_number; delete payload.request_code
  ...
  return <div className="max-w-7xl mx-auto">
    ...
    <div className="grid xl:grid-cols-[minmax(0,1fr)_360px] gap-7 items-start">
      <div className="space-y-5">
        <div className="bg-white rounded-2xl border border-[var(--color-deep-green)]/8 p-4 grid sm:grid-cols-2 lg:grid-cols-5 gap-3"><input value={filters.query} onChange={e => setFilters({ ...filters, query: e.target.value })} placeholder="Buscar código, título o cliente…" className="lg:col-span-1 rounded-xl border border-gray-200 px-3 py-2 text-sm" /><select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="all">Todos los estados</option>{Object.entries(REQUEST_STATES).map(([key, state]) => <option key={key} value={key}>{state.label}</option>)}</select><select value={filters.category} onChange={e => setFilters({ ...filters, category: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="all">Todas las temáticas</option>{Object.entries(REQUEST_CATEGORIES).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select value={filters.client} onChange={e => setFilters({ ...filters, client: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="">Todos los clientes</option>{clients.map(c => <option key={c.id} value={c.id}>{c.company || c.name}</option>)}</select><select value={filters.billing} onChange={e => setFilters({ ...filters, billing: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="all">Toda facturación</option>{Object.entries(BILLING_STATES).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
...
function RequestCard({ item, onEdit, onQuick, onNotify, onDelete, deleting }) { const state = requestState(item.status); const category = REQUEST_CATEGORIES[item.category] || REQUEST_CATEGORIES.other; const subRequests = Array.isArray(item.sub_requests) ? item.sub_requests : []; return <article className="bg-white rounded-2xl border border-[var(--color-deep-green)]/8 p-5 hover:shadow-sm transition-shadow"><div className="flex flex-col md:flex-row gap-4 justify-between"><div className="min-w-0"><div className="flex flex-wrap gap-2 items-center mb-2"><span className="inline-flex items-center rounded-full bg-[var(--color-deep-green)] text-white px-2.5 py-1 text-xs font-extrabold tracking-wide">{item.request_code || 'Sin código'}</span><span className={`inline-flex gap-1 items-center rounded-full border px-2.5 py-1 text-xs font-bold ${state.className}`}><span className="material-symbols-outlined text-sm">{state.icon}</span>{state.label}</span><span className="text-xs text-gray-400">{item.crm_clients?.company || item.crm_clients?.name || 'Sin cliente'} · {new Date(item.created_at).toLocaleDateString('es-AR')}</span></div><div className="flex items-center gap-2 mt-1"><h3 className="font-extrabold text-[var(--color-dark-gray)]">{item.title}</h3><span className="rounded-md bg-[var(--color-mint)]/35 px-2 py-0.5 text-[10px] font-bold text-[var(--color-deep-green)]">{category}</span></div><p className="text-sm text-gray-500 mt-1 whitespace-pre-wrap line-clamp-2">{item.description}</p>{subRequests.length > 0 && <div className="mt-3 rounded-xl bg-[var(--color-refined-gray)]/70 p-3"><p className="text-[10px] font-extrabold uppercase tracking-widest text-[var(--color-deep-green)]/65">{subRequests.length} sub-pedido{subRequests.length === 1 ? '' : 's'}</p><div className="mt-1 space-y-1">{subRequests.map((sub, index) => <p key={sub.position || index} className="text-xs font-semibold text-gray-600"><span className="text-[var(--color-deep-green)]">{index + 1}.</span> {sub.title || 'Sin título'}</p>)}</div></div>}{item.requester_name && <p className="mt-2 text-xs text-gray-500"><span className="material-symbols-outlined text-sm align-middle">person</span> Pedido por {item.requester_name}{item.requester_email ? ` · ${item.requester_email}` : ''}</p>}{item.affected_area && <p className="mt-2 text-xs text-[var(--color-deep-green)] font-semibold"><span className="material-symbols-outlined text-sm align-middle">web</span> {item.affected_area}</p>}{item.attachments?.length > 0 && <p className="mt-3 text-xs font-bold text-[var(--color-deep-green)]"><span className="material-symbols-outlined text-sm align-middle">attach_file</span> {item.attachments.length} archivo(s) adjunto(s)</p>}</div><div className="flex md:flex-col gap-2 shrink-0"><select value={item.status} onChange={e => onQuick(item, 'status', e.target.value)} className="rounded-lg border border-gray-200 px-2 py-1.5 text-xs font-bold">{Object.entries(REQUEST_STATES).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select>{item.status === 'in_progress' && <button type="button" onClick={onNotify} className="rounded-lg px-3 py-1.5 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-600 hover:text-white transition"><span className="material-symbols-outlined text-sm align-middle">mail</span> Notificar</button>}<Link to={`/admin/pedidos-cambios/${item.id}`} className="rounded-lg px-3 py-1.5 text-center text-xs font-bold bg-[var(--color-deep-green)]/7 text-[var(--color-deep-green)] hover:bg-[var(--color-deep-green)] hover:text-white transition">Ver y editar</Link><button type="button" onClick={onDelete} disabled={deleting} className="rounded-lg px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-600 hover:text-white transition disabled:opacity-50">{deleting ? 'Eliminando…' : 'Eliminar'}</button></div></div></article> }
