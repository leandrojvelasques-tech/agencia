...
export default function ChangeRequestsDashboard() {
  ...
  return <div className="max-w-7xl mx-auto">
    ...
    {notification && <NotificationModal notification={notification} onChange={setNotification} onClose={closeNotification} onCopy={copyNotification} onSend={sendNotification} sending={sendingNotification} />}
  </div>
}
...
function NotificationModal({ notification, onChange, onClose, onCopy, onSend, sending }) {
  return <div className="fixed inset-0 z-40 bg-slate-900/35 backdrop-blur-[2px] p-4 grid place-items-center" role="dialog" aria-modal="true" aria-labelledby="notification-title"><div className="w-full max-w-xl rounded-3xl bg-white shadow-2xl border border-[var(--color-deep-green)]/10 p-6"><div className="flex items-start justify-between gap-4 mb-5"><div><p className="text-[10px] uppercase tracking-widest font-bold text-blue-700/70">Envío directo</p><h2 id="notification-title" className="text-xl font-extrabold text-[var(--color-deep-green)]">Notificar que está en proceso</h2><p className="text-sm text-gray-500 mt-1">Se enviará desde info@leandrovelasques.com.ar.</p></div><button type="button" onClick={onClose} aria-label="Cerrar" className="text-2xl leading-none text-gray-400 hover:text-gray-700">×</button></div><div className="space-y-4"><AdminField label="Para"><input type="email" value={notification.to} onChange={e => onChange({ ...notification, to: e.target.value })} placeholder="correo@cliente.com" /></AdminField><AdminField label="Asunto"><input value={notification.subject} onChange={e => onChange({ ...notification, subject: e.target.value })} /></AdminField><AdminField label="Mensaje"><textarea rows="8" value={notification.message} onChange={e => onChange({ ...notification, message: e.target.value })} /></AdminField></div><div className="flex flex-col-reverse sm:flex-row justify-end gap-2 mt-6"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100">Cancelar</button><button type="button" onClick={onCopy} className="rounded-xl px-4 py-2 text-sm font-bold text-[var(--color-deep-green)] bg-[var(--color-mint)]/35 hover:bg-[var(--color-mint)]"><span className="material-symbols-outlined text-sm align-middle">content_copy</span> Copiar mensaje</button><button type="button" onClick={onSend} disabled={sending} className="btn-primary justify-center disabled:opacity-60"><span className="material-symbols-outlined text-sm">{sending ? 'progress_activity' : 'send'}</span> {sending ? 'Enviando…' : 'Enviar mail'}</button></div></div></div>
}
