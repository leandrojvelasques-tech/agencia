...
const emptyForm = { client_id: '', title: '', summary: '', description: '', affected_area: '', request_type: 'improvement', category: 'other', priority: 'normal', status: 'initiated', billing_status: 'pending', internal_note: '', client_note: '' }
...
export default function ChangeRequestsDashboard() {
  ...
  const filtered = useMemo(() => requests.filter(item => {
    const text = `${item.request_code || ''} ${item.title} ${item.summary || ''} ${item.description} ${item.requester_name || ''} ${item.requester_email || ''} ${item.crm_clients?.name || ''}`.toLowerCase()
  ...
  const save = async (event) => {
    ...
    if (!form.client_id || !form.title.trim() || !form.summary.trim() || !form.description.trim()) { notify('Completá cliente, título, síntesis y puntos a cambiar o implementar.', 'error'); return }
    setSaving(true)
